...
// down() method taking semaphore pointer
asmlinkage long sys_cs1550_down(struct cs1550_sem *sem)
{
	// set spinlock (from website)
	...
	if (sem->value < 0)
	{
		//set item to struct task_struct *
		sem->item = current;

		// OVERFLOW. -- If problematic, increase BUFFSIZE
		if (sem->rear == BUFF_SIZE - 1)
		{
		...
		else
		{
			...
			sem->rear = sem->rear + 1;
			// put item in queue array
			sem->queue_array[sem->rear] = sem->item;
		}
...
asmlinkage long sys_cs1550_up(struct cs1550_sem * sem)
{
	...
	{	
		...
		{
			printk("Queue Underflow \n");
			msleep(10000);
		}
		else
			// increase front value
			sem->front = sem->front + 1;		

		// wakeup dequeue'd process in queue array
		wake_up_process(sem->queue_array[(sem->front)-1]);
	}
	spin_unlock(&sem_lock);
	return 0;
}
